...
const supportedChainId = [ChainId.VANAR]
...
export function createFarmFetcher(multicallv2: MultiCallV2) {
  const fetchFarms = async (
    ...
    const masterChefAddress = isTestnet ? masterChefAddresses[ChainId.BSC] : masterChefAddresses[ChainId.VANAR]
  ...
  return {
    ...
    supportedChainId,
    isTestnet: (chainId: number) => ![ChainId.VANAR, ChainId.ETHEREUM].includes(chainId),
  }
}
