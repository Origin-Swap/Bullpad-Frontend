  ...
  return (
    <div>
      <div className="border rounded-lg py-4">
        <div className="mt-1">
          <div className="text-lg">Last 5 Add Liquidity Transactions</div>
          {userAccountData && userAccountData.liquidityTransactions && userAccountData.liquidityTransactions.length > 0 ? (
            <div className="grid grid-cols-3 gap-2 mt-4 border border-gray-100" style={{border: '2px solid gray', borderRadius: '10px', padding: '10px'}}>
              {/* Column Headers */}
              <div className="font-semibold" style={{borderRight: '2px solid gray', borderBottom: '2px solid gray', padding: '10px 5px 5px 5px'}}>Amount</div>
              <div className="font-semibold" style={{borderRight: '2px solid gray', borderBottom: '2px solid gray', padding: '10px 5px 5px 5px'}}>Token</div>
              <div className="font-semibold" style={{borderBottom: '2px solid gray', padding: '10px 5px 5px 5px'}}>TX Hash</div>

              {userAccountData.liquidityTransactions.map((transaction: any) => (
                <React.Fragment key={transaction.txhash}> {/* Use a unique identifier here */}
                  ...
                  <p className="text-sm" style={{ borderRight: '1px solid gray', borderBottom: '1px solid gray', padding: '0px 5px 5px 5px' }}>
                    {transaction.currencyA} / {transaction.currencyB}
                  </p>
