...
import { BULL, USDT } from '@pancakeswap/tokens'
...
const BAD_RECIPIENT_ADDRESSES: string[] = [
  '0x67f232ee36996cCb863f36E74B093e19851e24c7', // v2 factory
  '0xeeF9560CE1f1358011E41974dbE68F8a0Bd86E50', // v2 router 02
]
...
export function useDefaultsFromURLSearch():
  ...
  useEffect(() => {
    ...
    const parsed = queryParametersToSwapState(query, native.symbol, BULL[chainId]?.address ?? USDT[chainId]?.address)
...
export const useFetchPairPrices = ({
  ...
  // console.log('derivedPairData:', derivedPairData);
  ...
  useEffect(() => {
    const updatePairId = () => {
      try {
        ...
        // console.log('pairAddress:', pairAddress, token0Address, token1Address); // Debug statement
        if (pairAddress && pairAddress !== pairId) {
          setPairId(pairAddress);
        }
      } catch (error) {
        console.error('Error updating pairId:', error);
        setPairId(null);
      }
    };
  ...
  useEffect(() => {
        ...
        if (data) {
          ...
          try {
            pairTokenResults = await multicallv2({
              abi: IPancakePairABI,
              calls: [
                { address: pairId, name: 'token0' },
                { address: pairId, name: 'token1' }
              ],
              options: { requireSuccess: false }
            });
          } catch (error) {
            console.error('Error fetching tokenIds from pair:', error);
          }
          const newPairData =
            (pairTokenResults &&
          ...
          console.info(`[Price Chart]: No data available for ${pairId}`);
          dispatch(updatePairData({ pairData: [], pairId, timeWindow }));
          await fetchDerivedData();
        }
      } catch (error) {
        console.error('Failed to fetch pair prices:', error);
        dispatch(updatePairData({ pairData: [], pairId, timeWindow }));
        await fetchDerivedData();
      } finally {
        setIsLoading(false);
    ...
    const fetchDerivedData = async () => {
      console.info('[Price Chart]: Fetching derived prices');
      try {
    ...
    if (pairId && !isLoading && !pairData && !derivedPairData) {
      fetchAndUpdatePairPrice();
    }
  }, [pairId, timeWindow, pairData, token0Address, token1Address, derivedPairData, dispatch, isLoading]);

  const normalizedPairData = useMemo(() => {
    const normalized = normalizePairDataByActiveToken({ activeToken: token0Address, pairData });
    return normalized;
  }, [token0Address, pairData]);

  const normalizedDerivedPairData = useMemo(() => {
    const normalized = normalizeDerivedPairDataByActiveToken({ activeToken: token0Address, pairData: derivedPairData });
    return normalized;
  ...
  // console.log('fetchPairPrices:', pairPrices)
