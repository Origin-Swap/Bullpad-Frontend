...
const fetchTokenPriceData = async (
  ...
  try {
    ...
    if (blocksLength > 0 && chainName === 'SIRE' && !checkIsStableSwap()) {
    ...
    Object.keys(prices).forEach((priceKey) => {
      ...
      if (timestamp) {
        tokenPrices.push({
          timestamp,
          derivedETH: prices[priceKey]?.[`derived${mainToken}`]
      ...
      if (timestamp) {
        ...
        if (tokenPriceIndex >= 0) {
          ...
          tokenPrices[tokenPriceIndex].priceUSD =
            parseFloat(prices[priceKey]?.[`${mainToken.toLowerCase()}Price`] ?? 0) * derivedETH
        }
      }
    })
