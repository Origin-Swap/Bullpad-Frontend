...
export function useGasPrice(chainIdOverride?: number): string {
  ...
  const { data } = useFeeData({
    chainId,
    enabled: chainId !== ChainId.SIRE_TESTNET && chainId !== ChainId.SIRE_MAINNET,
    watch: true,
  })
  if (chainId === ChainId.SIRE_MAINNET) {
...
export function toV2LiquidityToken([tokenA, tokenB]: [ERC20Token, ERC20Token]): ERC20Token {
  return new ERC20Token(tokenA.chainId, Pair.getAddress(tokenA, tokenB), 18, 'Bull LPs', 'BullEx-LP')
}
