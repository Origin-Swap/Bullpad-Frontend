...
const getLpAddress = memoize(
  (token1: string | ERC20Token, token2: string | ERC20Token, chainId: number = ChainId.SIRE_TESTNET) => {
    let token1AsTokenInstance = token1
    let token2AsTokenInstance = token2
    console.log('Pair address:', token1AsTokenInstance, token2AsTokenInstance)
    if (!token1 || !token2) {
      return null
    }
    if (typeof token1 === 'string' || token1 instanceof String) {
      ...
      token1AsTokenInstance = new ERC20Token(chainId, checksummedToken1Address, 18, 'BullEx-LP')
    }
    if (typeof token2 === 'string' || token2 instanceof String) {
      ...
      token2AsTokenInstance = new ERC20Token(chainId, checksummedToken2Address, 18, 'BullEx-LP')
    }
