...
import { corechain } from '@pancakeswap/wagmi/chains'
...
export function getBlockExploreLink(
  ...
  const chainId = chainIdOverride || ChainId.SIRE_MAINNET
  const chain = chains.find((c) => c.id === chainId)
  if (!chain) return corechain.blockExplorers.default.url
  switch (type) {
    case 'transaction': {
...
export function getBlockExploreName(chainIdOverride?: number) {
  ...
  return chain?.blockExplorers?.default.name || corechain.blockExplorers.default.name
}

export function getBscScanLinkForNft(collectionAddress: string, tokenId: string): string {
  return `${corechain.blockExplorers.default.url}/token/${collectionAddress}?a=${tokenId}`
}
