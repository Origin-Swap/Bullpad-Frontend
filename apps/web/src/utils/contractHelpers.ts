...
import { BULL } from '@pancakeswap/tokens'
...
import dailyRewardAbi from 'config/abi/DailyRewardAbi.json'
...
export const getDailyRewardContract = (signer?: Signer | Provider, chainId?: number) => {
  return getContract({ abi: dailyRewardAbi, address: getDailyRewardAddress(chainId), signer })
}
...
export const getCakeContract = (signer?: Signer | Provider, chainId?: number) => {
  return getContract({
    abi: cakeAbi,
    address: chainId ? BULL[chainId].address : BULL[ChainId.CORE].address,
    signer,
  }) as Cake
}
