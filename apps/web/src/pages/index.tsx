...
export const getStaticProps: GetStaticProps = async () => {
  ...
  if (process.env.SF_HEADER) {
    try {
      ...
      if (
        totalTx?.uniswapFactory?.totalTransactions &&
        totalTx30DaysAgo?.uniswapFactory?.totalTransactions &&
        parseInt(totalTx.uniswapFactory.totalTransactions) > parseInt(totalTx30DaysAgo.uniswapFactory.totalTransactions)
      ) {
        results.totalTx30Days =
          parseInt(totalTx.uniswapFactory.totalTransactions) -
          parseInt(totalTx30DaysAgo.uniswapFactory.totalTransactions)
      }
    } catch (error) {
  ...
  const usersQuery = gql`
    query userCount($since: ISO8601DateTime, $till: ISO8601DateTime) {
      ethereum(network: core) {
        dexTrades(exchangeName: { in: ["BullEx", "BullEx v2"] }, date: { since: $since, till: $till }) {
          count(uniq: senders)
        }
      }
    }
